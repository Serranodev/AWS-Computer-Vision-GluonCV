...
def load_pretrained_classification_network():
    ...
    """
    # YOUR CODE HERE
    network = gcv.model_zoo.get_model('MobileNet1.0', root=M3_MODELS, pretrained=True)
    return network
    raise NotImplementedError()
...
def predict_probabilities(network, data):
    ...
    prediction = prediction[0] #We only had one input image,just remove the extra dimension from prediction
