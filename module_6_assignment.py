...
def load_image(filepath):
    ...
    :rtype: mx.nd.NDArray
    """
    img_file = mx.image.imread(filepath)    
    #raise NotImplementedError()
    return img_file
...
def transform_image(array):
    ...
    """
    normimage, unnormimage = gcv.data.transforms.presets.yolo.transform_test(array)

    #raise NotImplementedError()
    return normimage, unnormimage
...
def count_object(network, class_ids, scores, bounding_boxes, object_label, threshold=0.75):
    ...
    #scores[0] si es mayor que threshold agrego uno, class_ids[0] comparamos label, 
    ...
    for index,i in enumerate(class_ids):
        if class_names[i]==object_label:
            if scores[index]>=threshold:
                total_person_count+=1 
    
    num_people = total_person_count
    #raise NotImplementedError()
    return num_people
...
class PersonCounter():
    ...
    def count(self, filepath, visualize=False):
        image_output = load_image(filepath)
        norm_image, unnorm_image = transform_image(image_output)
